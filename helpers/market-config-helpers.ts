...
import zkConfig from "../markets/zksync";
import { getAllDeploymentsZK, getDeploymentAddress, save } from "./zkdeploy";
...
export const savePoolTokens = async (
  ...
  return Bluebird.each(Object.keys(reservesConfig), async (tokenSymbol) => {
    ...
    await save(
      hre.network.name,
      `${tokenSymbol}${ATOKEN_PREFIX}`,
      "InitializableAdminUpgradeabilityProxy",
      aTokenAddress
    );
    await save(
      hre.network.name,
      `${tokenSymbol}${VARIABLE_DEBT_PREFIX}`,
      "InitializableAdminUpgradeabilityProxy",
      variableDebtTokenAddress
    );
    await save(
      hre.network.name,
      `${tokenSymbol}${STABLE_DEBT_PREFIX}`,
      "InitializableAdminUpgradeabilityProxy",
...
export const getReserveAddresses = async (
  ...
  const allDeployments = await getAllDeploymentsZK(hre);
  const testnetTokenKeys = allDeployments.filter(
    (key) =>
  ...
  return testnetTokenKeys.reduce<ITokenAddress>((acc, key) => {
    ...
    acc[symbol] = getDeploymentAddress(hre, key);
    return acc;
  }, {});
};
...
export const getChainlinkOracles = async (
  ...
  const isLive = hre.config.networks[network].live || true;
  if (isLive) {
...
export const getTreasuryAddress = async (
  ...
  const deployedTreasury = await getDeploymentAddress(hre, TREASURY_PROXY_ID);
